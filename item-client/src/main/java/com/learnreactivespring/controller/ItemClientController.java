...
public class ItemClientController {
    ...
    public Flux<Item> getAllItemsUsingRetrieve(){
       return webClient.get().uri("/v1/items")
        .retrieve().bodyToFlux(Item.class).log("Items in Client Project retrieve");
    ...
    public Flux<Item> getAllItemsUsingExchange(){
        return webClient.get().uri("/v1/items")
                .exchange().flatMapMany(clientResp->clientResp.bodyToFlux(Item.class)).log("Items in Client Project exchange");
    ...
    public Mono<Item> getOneItemUsingRetrieve(){
        String id="5f7147ba8fdb5478436d6c46";
        return webClient.get().uri("/v1/items/{id}",id)
                .retrieve().bodyToMono(Item.class).log("Items in Client Project retrieve single Item");

    }

    @GetMapping("/client/exchange/singleItem")
    public Mono<Item> getOneItemItemsUsingExchange(){
        String id="5f7147ba8fdb5478436d6c46";

        return webClient.get().uri("/v1/items/{id}",id)
                .exchange().flatMap(clientResp->clientResp.bodyToMono(Item.class)).log("Items in Client Project retrieve single Item");
